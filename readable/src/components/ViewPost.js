...
class ViewPost extends Component{
  ...
  // To handle submission of new comment
  handleSubmit = (e) => {
    e.preventDefault()
  ...
  render(){
    ...
    return(
      <Grid>
        <Row>
        <Link to="/"><Button bsStyle="success"> Back </Button></Link>
        </Row>
        {post ?
        <Row>
            ...
            <Form onSubmit={this.handleSubmit}>
              ...
              <FormGroup>
                <FormControl type="text" name="author"
                 ...
                 pattern="^\S.*$" title="Author is a required field"/>
              </FormGroup>
        ...
        <ListGroup className="media-list width-fix">
          {filteredComments && filteredComments.map(comment =>
            ...
            <ListGroupItem key={comment.id} className="media mt-4">
              ...
              <p>
                <Button bsStyle="success" className="mr-5" onClick={() => this.editAComment(comment.id)}>
                  Save
                </Button>
            ...
            <ListGroupItem key={comment.id} className="media mt-4">
              <div>
                <div className="media-left">
                  <p>
                    <Button bsStyle="success" onClick={() => this.updateCommentVote(comment.id, 'upVote')}>
                      <Glyphicon glyph="triangle-top"/>
                    </Button>
                  </p>
                  <p className="comment-score">{comment.voteScore}</p>
                  <p>
                    <Button bsStyle="success" onClick={() => this.updateCommentVote(comment.id, 'downVote')}>
